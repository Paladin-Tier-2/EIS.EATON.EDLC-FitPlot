...
% MANUAL EDITING -- Ideally 0% but oh well.
target_soc_for_annotation = 20; 
target_freqs = [0.010,0.020,0.1, 1, 10, 100]; 
...
fig = gcf;                                      % handle to the current figure
forceHeadTimer = timer( ...                     
    'ExecutionMode','fixedSpacing', ...         % run at a fixed, regular period
    'Period',1, ...                             % …that period is 1 s
    'TimerFcn',@(~,~) ...
        set(findall(fig,'Type','Quiver'), ...   % locate *all* quiver objects in this fig
            'MaxHeadSize',0.05));                % and pin their MaxHeadSize property to 0.1
start(forceHeadTimer);                          % begin the repeating task
set(fig,'DeleteFcn',@(~,~) delete(forceHeadTimer)); % auto-clean when the figure is closed

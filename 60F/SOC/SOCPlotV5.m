...
% --- This is the NEW "Centroid Repulsion" block ---
if target_dataset_idx > 0
    % --- Step 1: Identify the target data point ---
    target_dataset = soc_datasets{target_dataset_idx}
    [~, target_idx] = min(abs(target_dataset.freq - target_freq))
    point_to_annotate = [target_dataset.real(target_idx), target_dataset.imag(target_idx)]

clear; clc; close all;
pub_options.arrowLineWidth = 0.75;   % make it visible for the first arrow call
prompt = false;
...
target_freqs = [0.015,0.1, 1, 10, 100]; 
...
if target_dataset_idx > 0
    ...
    for f_idx = 1:length(target_freqs)
        ...
        [nxTxt, nyTxt] = data2norm(gca, x_text, y_text);              % data→normalised for the text location
        [nxAnc, nyAnc] = data2norm(gca, original_anchor(1), ...
                                original_anchor(2));          % …and for the anchor point
        h_arrow = annotation('arrow', ...                             % true figure-space arrow object
            [nxTxt nxAnc], [nyTxt nyAnc], ...                         % [xStart xEnd], [yStart yEnd]
            'Color','k', ...                                          % black line (same as quiver default)
            'LineWidth', pub_options.arrowLineWidth, ...              % preserve your publication style
...
function [norm_x, norm_y] = data2norm(ax, data_x, data_y)
    ...
    % ---------- NEW : make X behave like Y did ----------
    if strcmp(get(ax,'XDir'),'reverse')          % flip if axis is reversed
        norm_x_in_ax = 1 - norm_x_in_ax;
    end
    if strcmp(get(ax,'YDir'),'reverse')          % already added earlier
        norm_y_in_ax = 1 - norm_y_in_ax;
    end

    norm_x_in_ax = max(0, min(1, norm_x_in_ax));
    norm_y_in_ax = max(0, min(1, norm_y_in_ax));

    norm_x = ax_pos(1) + norm_x_in_ax * ax_pos(3);
    norm_y = ax_pos(2) + norm_y_in_ax * ax_pos(4);
    % clamp so arrows never fly outside the axes
...
% UPGRADED with a live HUD to monitor critical arrow properties.
    ...
    [nxTxt, nyTxt] = data2norm(ax, new_text_pos(1), new_text_pos(2));     % new text pos → normalised
    [nxAnc, nyAnc] = data2norm(ax, final_anchor_point(1), ...
                                 final_anchor_point(2));               % new anchor → normalised
    set(h_arrow, 'X',[nxTxt nxAnc], 'Y',[nyTxt nyAnc]);         
        set(text_handle, 'Position', [new_text_pos, 0]);

    % --- 4. UPDATE THE DEBUG HUD (The new part) ---
    h_hud = findobj(gcf, 'Tag', 'debug_hud');
    if ~isempty(h_hud)
        head_len  = get(h_arrow,'HeadLength');                   % annotation-specific
        head_wid  = get(h_arrow,'HeadWidth');
        xy_norm   = get(h_arrow,{'X','Y'});                      % two-element cell → [xStart xEnd], [yStart yEnd]
        arrow_vector_length = hypot(diff(xy_norm{1}), ...
                                    diff(xy_norm{2}));           % length in normalised units
        
        debug_str = { ...
            '--- LIVE ARROW STATS ---', ...
            sprintf('HeadLength: %d pt', head_len), ...
            sprintf('HeadWidth : %d pt', head_wid), ...
            sprintf('LineWidth : %.2f', line_width_val), ...
            sprintf('VectorLen : %.3f (norm.)', arrow_vector_length) ...
        };

...
if target_dataset_idx > 0
    ...
    % --- FIX ---
    % Create the text box and arrow separately.
    % This is the correct way to get a boxed label with an arrow.

    % 1. Create the text box. This creates the text and the box around it.
    h_text = annotation('textbox', [x_tail, y_tail, 0, 0], ...
        'String', label_text, ...
        'FontSize', 16, ...
        'FontWeight','bold',...
        'FitBoxToText', 'on', ...
    ...
    % 2. Get the final position of the text box we just made.
    %    MATLAB adjusts the position automatically based on text size.
    text_pos = get(h_text, 'Position');

    % 3. Define the start and end points for a simple arrow.
    %    Start point: Center of the bottom edge of the text box.
    arrow_start_x = text_pos(1) + text_pos(3) / 2;
    arrow_start_y = text_pos(2);
    %    End point: The data point.
    arrow_end_x = x_head;
    arrow_end_y = y_head;

    % 4. Create the arrow object. This draws ONLY the arrow, no text.
    annotation('arrow', [arrow_start_x, arrow_end_x], [arrow_start_y, arrow_end_y]);
end

...
if target_dataset_idx > 0
    ...
    for f_idx = 1:length(target_freqs)
        ...
        % --- 3. Create the graphical objects (text then arrow) -------------
        h_text = text(x_text, y_text, text_label, ...          % was created after quiver
                'FontSize',9, ...
                'BackgroundColor','w','Margin',0.2,'EdgeColor','k', ...
                'Tag','draggable_annotation');


        ext   = get(h_text,'Extent');                       % [x y w h]  box
        bx    = ext(1);  by = ext(2);                       % lower-left
        bw    = ext(3);  bh = ext(4);
        
        % --- closest point on box perimeter to the anchor -----------------------
        tail_x = min( max(original_anchor(1), bx) , bx+bw );
        tail_y = min( max(original_anchor(2), by) , by+bh );
        
        h_arrow = quiver(tail_x-bw, tail_y-bh, ...
                         original_anchor(1)-tail_x, ...
                         original_anchor(2)-tail_y, 0, ...
                         'LineWidth',1,'Color','k','HandleVisibility','off');
...
% This version switches the arrow's target between two pre-defined points.
    ...
    set(text_handle,'Position',[new_text_pos,0],'Units','data');  % ensure data units
    
    ext         = get(text_handle,'Extent');            % updated box
    bx = ext(1);  by = ext(2);  bw = ext(3);  bh = ext(4);
    
    tail_x = min( max(final_anchor_point(1), bx) , bx+bw );
    tail_y = min( max(final_anchor_point(2), by) , by+bh );

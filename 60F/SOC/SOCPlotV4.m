...
if target_dataset_idx > 0
    ...
    xc  = mean(xlim);                    % centre of current axes
    yc  = mean(ylim);
    
    v   = [x_p - xc , y_p - yc];         % centre → point
    v   = v ./ norm(v);                  % unit vector
    
    offset  = 0.10 * hypot(diff(xlim), diff(ylim));   % 10 % of plot diagonal
    x_lbl   = x_p + offset * v(1);       % label position (data units)
    y_lbl   = y_p + offset * v(2);
    
    quiver(x_lbl, y_lbl, x_p - x_lbl, y_p - y_lbl, 0, ...
           'LineWidth',1.5, 'MaxHeadSize',0.25, 'Color','k');
    
    text(x_lbl, y_lbl, sprintf('%.0f Hz', target_freq), ...

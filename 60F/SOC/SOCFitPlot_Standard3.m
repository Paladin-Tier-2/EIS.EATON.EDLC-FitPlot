...
h_dummy_measured = plot(nan, nan, 'w'); % Invisible, white line for header
h_dummy_fit      = plot(nan, nan, 'w'); % Invisible, white line for header
...
for k = 1:length(socFolders)
    ...
    if ~isempty(tokens)
        ...
        socPercentagesFitted{end+1} = sprintf('%3d%%', socValues(k));
...
visible_header_meas = 'Measured';
visible_header_fit  = 'Fit';

padding_len = length(visible_header_meas) - length(visible_header_fit);
header_fit_padded = [visible_header_fit, repmat(' ', 1, padding_len)];

header_meas = '\bf{Measured}';
header_fit  = sprintf('\\bf{%s}', header_fit_padded);


all_handles = [h_dummy_measured, h_measured, h_dummy_fit, h_fitted];
all_labels  = [header_meas, socPercentagesMeasured, header_fit, socPercentagesFitted];

% Create the legend. It automatically uses the DisplayName of each handle.
leg = legend(all_handles, all_labels, 'Location', 'northwest');
leg.NumColumns = 1;

% --- Final Styling for the Legend ---
set(leg, 'FontName', 'Courier New'); % Monospaced font for number alignment
set(leg, 'Interpreter', 'tex');     % To render the bold '\bf' command

leg.ItemTokenSize   = [10 10];   % ① shrink the icon-plus-gap to 8 pt × 8 pt
leg.IconColumnWidth = 10;       % ② reserve only 8 pt in front of each label (R2024b+)
...
if isZoomed
    outputFileName = sprintf('%s/SOC-Fit_%sF_Publication.pdf', FiguresFol, capNumber);
else
    outputFileName = sprintf('%s/SOC-%sF_Fit_Publication_FULL_VIEW.pdf', FiguresFol, capNumber);
end
...
fig.set('SizeChangedFcn',@figureCallback)

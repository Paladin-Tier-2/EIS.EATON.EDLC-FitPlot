...
markFrequencies = [31600, 630, 2, 15e-3, 100e-3];  % Hz
% Matching tolerance for each tracked frequency.
tolFreq = [1e3, 50, 0.5, 1e-3, 10e-3];  % Hz
markedPoints = cell(length(markFrequencies), 1);

% Store data so the data cursor/frequency tracking can be reused later.
frequencyData = {};
allData = {};
...
for k = 1:length(socFolders)
    ...
    markedPoints = collectMarkedPoints(markedPoints, markFrequencies, tolFreq, freq, ...
...
outputFileName = fullfile(figuresFolder, sprintf('SOC-Fit%sF_Full.pdf', capNumber));
...
function markedPoints = collectMarkedPoints(markedPoints, markFrequencies, tolFreq, freq, real_part, imaginary_part, SOC)
%COLLECTMARKEDPOINTS Store points close to selected frequencies.
%
% The points are not drawn in this fit plot, but the original script printed
% these matches while it ran.
